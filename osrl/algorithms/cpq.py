...
class CPQ(nn.Module):
    ...
    def critic_loss(self, observations, next_observations, actions, rewards, done):
        ...
        with torch.no_grad():
            ...
            backup = rewards + self.gamma * (1 - done) * (qc_targ <= self.q_thres) * q_targ
    ...
    def cost_critic_loss(self, observations, next_observations, actions, costs, done):
        ...
        with torch.no_grad():
            ...
            # sample actions
            sampled_actions = pi_dist.sample([self.sample_action_num])  # [sample_action_num, batch_size, act_dim]
            sampled_actions = sampled_actions.reshape(self.sample_action_num*batch_size, self.action_dim)
            stacked_obs = torch.tile(observations[None, :, :], (self.sample_action_num, 1, 1))  # [sample_action_num, batch_size, obs_dim]
            stacked_obs = stacked_obs.reshape(self.sample_action_num*batch_size, self.state_dim)
...
class CPQTrainer:
    ...
    def train_one_step(self, observations, next_observations,
        ...
        # update critic
        loss_critic, stats_critic = self.model.critic_loss(observations, next_observations, actions, rewards, done)
        # update cost critic
        loss_cost_critic, stats_cost_critic = self.model.cost_critic_loss(observations, next_observations, actions, costs, done)
        # update actor
